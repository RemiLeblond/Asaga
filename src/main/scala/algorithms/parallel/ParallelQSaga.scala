...
/**
  * Parallel, sparse qSaga (see http://arxiv.org/abs/1506.03662)
  * For parameter details, see StochasticGradientDescent
  */
abstract class ParallelqSaga(inputs: CSRMatrix,
                             output: Array[Double],
                             stepSize: Double,
  ...
  def run(printLosses: Boolean, verbose: Boolean) {
    ...
    for (i <- 1 to iterations + 1) {
      ...
      // take just one step
      val (index, scalar, oldScalar) = singleSteps(0)
...
class ParallelqSagaLS(inputs: CSRMatrix,
                      output: Array[Double],
                      stepSize: Double,
                      ...
                      cores: Int,
                      lambda: Double)
  extends ParallelqSaga(inputs, output, stepSize, iterations_factor, historyRatio, cores, lambda) with LeastSquares


class ParallelqSagaLogistic(inputs: CSRMatrix,
                            output: Array[Double],
                            stepSize: Double,
                            iterations_factor: Int,
                            historyRatio: Int,
                            cores: Int,
                            lambda: Double)
  extends ParallelqSaga(inputs, output, stepSize, iterations_factor, historyRatio, cores, lambda) with LogisticRegression


object ParallelqSaga {
  def main(args: Array[String]) {
    if (args.size != 7) {
      println("Usage: ParallelqSaga <data><model><stepsize><iterations><history><cores><lambda>")
      System.exit(0)
    }
    val (inputs, output) = args(0) match {
      case "rcv1" => loadDataRCV1()
    }
    val (stepSize, iterationsFactor, historyRatio, cores, lambda) = (args(2).toDouble, args(3).toInt, args(4).toInt, args(5).toInt, args(6).toDouble)

    val sls = args(1) match {
      case "LS" => new ParallelqSagaLS(inputs, output, stepSize, iterationsFactor, historyRatio, cores, lambda)
      case "Logistic" => new ParallelqSagaLogistic(inputs, output, stepSize, iterationsFactor, historyRatio, cores, lambda)
    }
